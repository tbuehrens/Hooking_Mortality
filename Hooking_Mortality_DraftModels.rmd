...
packages_list<-c("tidyverse","magrittr","lubridate","scales","RODBC","MuMIn","RColorBrewer","reshape2","gplots","mgcv","sjPlot","sjmisc","gridExtra","modelr","kableExtra","dataRetrieval","MASS","brms","rstan","sf","riverdist","maptools","rgdal","raster","rgeos","devtools","webshot","phantom","marginaleffects", "brmsmargins", "here","lemon", "HDInterval") 
...
catvars = dat %>%
  ...
  names() %>%
  c("CritHook_cap", "HookType_reduced", "Method_reduced", "Method_Gear","GearBarbHook")
...
dat %<>%
  ...
  HookType_reduced = ifelse(HookType_cap %in% c("Double","Treble"),"Multi","Single"),
  ...
  GearBarbHook = ifelse(is.na(Gear_cap) | is.na(Barb_cap) | is.na(HookType_reduced), NA,  paste(Gear_cap, Barb_cap, HookType_reduced, sep = "_"))) %>%
...
Species.labs <- c('COHO'="Coho", 'SCHK'="Spring Chinook", 'STLHD'= "Summer Steelhead", 'WSTLHD'= "Winter Steelhead")

#Time to recapture by species, and treatment/control
ttr_fig = dat %>%
  filter(SpeciesCode_cap %in% c("COHO","SCHK","WSTLHD","STLHD") & Treatment_cap == "Treatment") %>%
  mutate(Days_atlarge = difftime(as.Date(Survey_Date_recap), as.Date(Survey_Date_cap), units = "days")) %>%
  ggplot(aes(Days_atlarge)) +
  labs(x="Days-at-Large", y="Number of Fish")+
  geom_bar(fill="black")+
  scale_x_continuous(breaks = c(1,seq(15,180,by = 15)))+
  facet_rep_wrap(~SpeciesCode_cap, scales = "free_y",
             labeller = labeller(SpeciesCode_cap=Species.labs))+
...
cdat<-dat%>%
  ...
  filter(Fish_ID != 375) #removing the single bobber/lure fish
...
post_summary = post %>%
  mutate(neg = ifelse(value < 0,1,0)) %>%
  group_by(name) %>%
  dplyr::summarise(mean = mean(value),
                   median = median(value),
                   hdi_95L = HDInterval::hdi(value)[1],
                   hdi_95U = HDInterval::hdi(value)[2],
                   prob_neg = sum(neg)/4000) %>%
  ungroup()
...
```{r Coho-fullmodel-results, results="hold",echo = F, message = T, warning = FALSE}

post_summary %>%
  dplyr::arrange(desc(prob_neg)) %>%
  mutate_if(is.numeric,
            round,
            digits = 4) %>%
  kable(booktabs = T,
         col.names = c("Covariate", "Mean","Median","95% HDI, lower", "95% HDI, upper", "P(Negative effect)")) %>%
  kable_styling(position = "center",
                bootstrap_options = c("striped", "condensed"))
...
priorname = c("hs","norm")[1]
re = c("(1| survey_ID_cap)", "s(DOY_cap,run_year_cap,bs='fs')")
covars = c("Gear_cap","Barb_cap","HookType_reduced")
cov_form = c("Gear_cap","Barb_cap","HookType_reduced")
formula = bf((paste('rec ~ 1 + ', paste(re, collapse = ' + '),'+', paste(cov_form, collapse = ' + '))))
...
brm_coho = brms::brm(formula = formula
          ...
          ,prior = prior(horseshoe(df = 1, par_ratio = 0.5), class = "b")
          #,prior = prior(normal(0,16), class = "b")#Test on more restricted ind prior
    )
...
surv_quant = surv_dist %>%
  ...
  dplyr::summarize(mean = mean(surv),
                   median = quantile(surv, .5),
                   hdi_95l = HDInterval::hdi(surv, .95)[1],
                   hdi_95u = HDInterval::hdi(surv, .95)[2]
  ) %>%
  ungroup()
...
surv_quant %>%
  kable(col.names = c("Gear_Barb_Hook Type", "Mean","Median","95% HDI, lower","95% HDI, upper"), booktabs = T,
        caption = "Estimated relative survival for coho angled with gear, barb, and hook type combinations.") %>%
  kable_styling(position = "center",
                bootstrap_options = c("striped", "condensed"))
...
meff_quant = surv_dist %>%
  ...
  dplyr::summarize(mean = mean(value),
                   med = median(value),
                   hdi_95l = HDInterval::hdi(value,.95)[1],
                   hdi_95u = HDInterval::hdi(value,.95)[2]
  ) %>%
  ungroup()
...
count = cdat_treat %>%
  dplyr::select(Fish_ID, Method_Gear) %>%
  group_by(Method_Gear) %>%
  dplyr::summarise(count = n_distinct(Fish_ID)) %>%
  ungroup() %>%
  right_join(surv_dist, by = c("Method_Gear" = "name")) %>%
  rename(name = Method_Gear) %>%
  mutate(count = paste0("n = ",count)) %>%
  mutate(across('name', str_replace, '_',' ')) %>%
  group_by(name, count) %>%
  dplyr::summarise(
    upperw = quantile(value,0.75)+1.5*(quantile(value,0.75)-quantile(value,0.25)+.005)
  )
...
critfig = surv_dist %>%
  ...
  ggplot(aes(order, value))+
  geom_boxplot(outlier.shape = NA)+
  geom_text(data = count, aes(x= name, label = count,
                              y = upperw))+
  #stat_summary(fun.y=mean, geom="point", size = 2, color = "red")+
  labs(y = "Probability of Critical Hooking Location")+
...
meff = bind_cols(brmsmargins(brm_cohohand,
                 ...
                 brmsmargins(brm_cohohand,
                   ...
                   rename(NonCritical_hook = V1,
                          Critical_hook = V2),
                 brmsmargins(brm_cohohand,
                        at = tibble(HookType_reduced = c("Single","Multi")),
                        effects = "fixedonly",
                        CI = .95)$Posterior %>%
                         as.tibble() %>%
                   rename(Singlehook = V1,
                          Multihook = V2)) %>%
  mutate(barb_eff = Barbed_hook - Barbless_hook,
         crit_eff = Critical_hook - NonCritical_hook,
         multihook_eff = Multihook - Singlehook) %>%
  dplyr::select(barb_eff, crit_eff, multihook_eff) %>%
  pivot_longer(cols = everything()) %>%
  mutate(pos = ifelse(value > 0, 1,0)) %>%
  group_by(name) %>%
  dplyr::summarise(mean = mean(value),
  ...
  #        med = round(med,1),
  #        hdi_95l = round(hdi_95l - avghandle,1),
  #        hdi_95u = round(hdi_95u - avghandle,1)) %>%
  dplyr::select(name, med, mean, hdi_95l, hdi_95u, p.pos) %>%
  rename(` ` = name,
         `Median effect on handling time (seconds)` = med,
         `Mean effect on handling time (seconds)` = mean,
         `95% HDI, lower` = hdi_95l,
         `95% HDI, upper` = hdi_95u,
         `P(positive)` = p.pos)
...
meff %>%
  kable(booktabs = T,
        caption = "Predicted effect on handling time (mean handling time = 95 seconds)") %>%
  kable_styling(position = "center",
                bootstrap_options = c("striped", "condensed"))
...
```{r BRMS analysis SCHK results, results="hold",echo = F, message = T, warning = FALSE }
...
brm_schk = brms::brm(formula = formula
          ...
          ,control = list(adapt_delta = .95)
          ,prior = prior(horseshoe(df = 1, par_ratio = 0.5), class = "b")
          #,prior = prior(normal(0,16), class = "b")#Test on more restricted prior
    )
...
schkreg_surv = surv_dist %>%
  mutate(Species = "Spring Chinook")

surv_quant = surv_dist %>%
  group_by(name) %>%
  dplyr::summarize(mean = mean(surv),
                   median = median(surv, .5),
                   hdi_95l = HDInterval::hdi(surv, .95)[1],
                   hdi_95u = HDInterval::hdi(surv, .95)[2]
  ) %>%
  ungroup()
...
surv_quant %>%
  kable(col.names = c("Gear_Barb_Hook Type", "Mean","Median","95% HDI, lower","95% HDI, upper"), booktabs = T,
        caption = "Estimated relative survival for spring chinook angled with gear, barb, and hook type combinations.") %>%
  kable_styling(position = "center",
                bootstrap_options = c("striped", "condensed"))
...
## Hooking mortality figure
```{r hooking-mort-fig-coho-schk, results = 'hold', echo = F, message = T, warning = F}
...
hm_fig = bind_rows(cohoreg_surv, schkreg_surv) %>%
  mutate(HookType_reduced = ifelse(HookType_reduced == 'Multi', '\n Multi-hook', '\n Single Hook')) %>%
  mutate(hm = (1 - surv)*100,
         gearhook = as.factor(paste(paste0(Gear_cap,','),HookType_reduced, sep = ' '))) %>%
  mutate(gearhook = fct_relevel(gearhook, "Jig, \n Single hook", "Lure, \n Single Hook", "Lure, \n Multi-hook", "Bait, \n Single Hook", "Bait, \n Multi-hook")) %>%
  ggplot(aes(x = gearhook, y = hm, fill = Barb_cap)) +
  stat_summary(fun.data = calc_boxplot_stat, geom="boxplot"
               ,width = .75
               ,position = position_dodge2(preserve = 'single'))+
  #geom_boxplot(width = .75,outlier.shape = NA,position = position_dodge(preserve = 'single'))+
  scale_fill_manual(labels=c("Barbless","Barbed"), values = c("white","grey60"))+
  ...
  labs(y="Hooking Mortality (%)")+
  facet_rep_wrap(~Species, scales = 'free',
                 labeller = labeller())

hm_fig

tiff(paste0(here(),'/figures/hm_cohoschk_fig.tiff'), res = 300, compression = 'lzw', width = 6.5, height = 4, units = 'in')
hm_fig
dev.off()
...
```{r BRMS analysis steelhead-combined-results, results = "hold", echo = F, message = T, warning = FALSE}
...
```{r BRMS analysis steelhead-regmodel-results, results="hold",echo = F, message = T, warning = FALSE}


rec_fig = surv_dist %>%
  mutate(Gear_cap = ifelse(Gear_cap == "Fly","Yarn",Gear_cap),
         HookType_reduced = ifelse(HookType_reduced == 'Multi', '\n Multi-hook', '\n Single Hook')) %>%
  mutate(gearhook = as.factor(paste(paste0(Gear_cap,','),HookType_reduced, sep = ' '))) %>%
  mutate(gearhook = fct_relevel(gearhook, "Jig, \n Single Hook", "Lure, \n Single Hook", "Lure \n Multi-hook", "Bait, \n Single Hook", "Bait, \n Multi-hook", "Yarn, \n Single Hook", "Yarn, \n Multi-hook")) %>%
  ggplot(aes(x = gearhook, y = value, fill = Barb_cap)) +
  geom_boxplot(width = .75,outlier.shape = NA,position = position_dodge2(preserve = 'single'))+
  scale_fill_manual(labels = c("Barbless","Barbed"), values = c("white","grey60"))+
  #coord_cartesian(ylim = c(0,80))+
  scale_y_continuous(limits = c(0.35, 0.72))+
  theme(axis.text = element_text(color = 'black'),
        legend.title = element_blank(),
        strip.background=element_blank(), 
        panel.grid = element_blank(), panel.background = element_blank(),
  ...
  labs(y="Recapture Probability")

rec_fig

tiff(paste0(here(),'/figures/rec_sthd_fig.tiff'), res = 300, compression = 'lzw', width = 6.5, height = 4, units = 'in')
rec_fig
dev.off()
