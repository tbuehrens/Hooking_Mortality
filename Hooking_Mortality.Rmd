...
cdat<-dat%>%
  ...
  mutate(across(c(Gear_cap, Barb_cap), ~tidyr::replace_na(., "Control")))%>%
...
m1<-gam(rec ~ 
          ...
          # + s(DOY_cap,run_year_cap,bs="fs",m=2)
          # + s(survey_ID_cap,bs="re")

          # Treatment effects
          + Gear_cap : Treatment_cap
          + Barb_cap : Treatment_cap
...
cdat%>%group_by(Treatment_cap,Gear_cap,Barb_cap)%>%tally()
...
ggplot(newdat,aes(x=Gear_cap,fill=Barb_cap, y=HM))+
  geom_crossbar(aes(ymin = HM_U50, ymax = HM_U50), width = 0.2)
  
# 
# 
# ggplot(newdat,aes(x=DOY_cap,y=preds,group=run_year_cap,group=Treatment_cap))+
#   geom_line(size=0.8)+
#   facet_wrap(~Treatment_cap)+
#   ylim(0,1)+
