...
cdat<-dat%>%
  ...
  filter(!Gear_cap%in%c("Baited Lure")) #F
...
p<-ggplot(newdat, aes(x=Gear_cap, fill=Barb_cap)) +
  geom_boxplot(aes(ymin = HM_L95, lower = HM_L50, middle = HM, upper = HM_U50, ymax = HM_U95),stat="identity")+
  geom_hline(yintercept=0)+
  ylab("Hooking Mortality")
print(p)
ggsave("Coho_HM.png",p,width = 5, height = 3.5, units = "in")


p<-ggplot(newdat%>%mutate(across(c(HM,HM_L50,HM_U50,HM_L95,HM_U95),~ifelse(. < 0,0,.)))
          , aes(x=Gear_cap, y=HM, color=Barb_cap)) +
  geom_errorbar(aes(ymax = HM_U95,ymin = HM_L95,color=Barb_cap),size=1,position = position_dodge(0.3),width = 0.2)+
  geom_linerange(aes(ymax = HM_U50,ymin = HM_L50,color=Barb_cap),size=2.5,position = position_dodge(0.3))+
  geom_point(aes(color=Barb_cap),size=8,position = position_dodge(0.3))+
...
print(p)
ggsave("Coho_HM_pointrange.png",p,width = 5, height = 3.5, units = "in")
...
newdat<-expand.grid(
  ...
  as_tibble()%>%
  filter(!(Gear_cap == "Lure" & Barb_cap =="No")) #no fish caught with un-barbed lures
...
p<-ggplot(newdat%>%filter(Gear_cap!="F"), aes(x=Gear_cap, fill=Barb_cap)) +
  geom_boxplot(aes(ymin = HM_L95, lower = HM_L50, middle = HM, upper = HM_U50, ymax = HM_U95),stat="identity")+
  geom_hline(yintercept=0)+
  ylab("Hooking Mortality")

print(p)
ggsave("S_Stlhd_HM.png",p,width = 5, height = 3.5, units = "in")
...
print(p)
ggsave("W_Stlhd_HM.png",p,width = 5, height = 3.5, units = "in")
```
...
print(p)
ggsave("S_Chnk_HM.png",p,width = 5, height = 3.5, units = "in")
