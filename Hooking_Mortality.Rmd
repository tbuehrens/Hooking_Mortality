...
formula = as.formula(paste('rec ~ 1 + ', paste(re, covars, collapse = ' + ')))
...
#save results
saveRDS(brm_coho, file = here(paste0("results/", modname,"_",priorname,".rds")))
}else{
  brm_coho<-readRDS(here(paste0("results/", modname,"_",priorname,".rds")))
}
...
```{r BRMS analysis SPCK, results="hold",echo = T, message = T, warning = FALSE }
treatvars = dat %>%
  dplyr::select(FightTime_seconds_cap:AnglerCode_cap, HookLeftinFish_cap) %>%
  names()
...
plot(conditional_effects(brm2), points = T, ask = F)

meff = brmsmargins(brm2, 
                   at = data.frame(Gear_cap = c("Control", rep(c("Bait","Lure"),2)), Barb_cap = c("Control",rep(c("No","Yes"), 2))),
                   effects = "integrateoutRE",
                   seed = 425
                   ) 
