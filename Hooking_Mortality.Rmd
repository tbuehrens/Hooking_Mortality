...
packages_list<-c("tidyverse","lubridate","scales","RODBC","MuMIn","RColorBrewer","reshape2","gplots","mgcv","sjPlot","sjmisc","gridExtra","modelr","kableExtra","dataRetrieval","MASS","brms","rstan","sf","riverdist","maptools","rgdal","raster","rgeos","devtools","webshot","phantom","marginaleffects", "brmsmargins", "here") 
...
```{r data-management-for-analysis}
  treatvars = dat %>%
  dplyr::select(TimeCapture_cap:AnglerCode_cap, HookLeftinFish_cap) %>%
  names()
...
dat %<>%
  ...
  dplyr::mutate(across(all_of(treatvars), ~ ifelse(Treatment_cap == "Control", "Control",.)))%>%
  ...
  mutate(FightTime_seconds_cap = as.numeric(ifelse(FightTime_seconds_cap == "Control", 0, FightTime_seconds_cap))
         ...
         ,CritHook_cap = ifelse(HookLocation_cap == "Control", "Control", ifelse(HookLocation_cap %in% c("G","E","T","S"), "Critical","Non-critical")))
...
modname = c("treatonly","crithook","treat_gear","gear_barb", "treat_gear_barb","gear:barb_crithook", "treat_gear:barb_crit_fungus", "treat_crithook_REtemp_fl_condtion", "fullCat","full")[2]
priorname = c("norm","hs", "hs_secondaryeffs")[2]
re = c("(1| survey_ID_cap)", "s(DOY_cap,run_year_cap,bs='fs')")
covars = c("CritHook_cap")
cov_form = c("CritHook_cap")
formula = bf((paste('rec ~ 1 + ', paste(re, collapse = ' + '),'+', paste(cov_form, collapse = ' + '))))
# formula = bf(rec ~ 1 + (1| survey_ID_cap) + s(DOY_cap,run_year_cap,bs='fs') + a + b, nl = T) +
#   lf(a ~ Gear_cap:Barb_cap, center = T) +
#   lf(b ~ 0 + Treatment_cap, cmc = F)
...
surv_quant = surv_dist %>%
  group_by(name) %>%
  dplyr::summarize(med = quantile(surv, .5),
...
if("Treatment_cap" %in% covars){
  treat_plot = ggplot(data = surv_dist, aes(x = Treatment_cap, y = surv))+
    geom_violin()+
    geom_boxplot()+
    coord_cartesian(ylim = c(.9,1.1))
}

if("Gear_cap" %in% covars){
  if("HookSize_cap" %in% covars){
    gear_hooksize_plot = ggplot(data = surv_dist, aes(x = Gear_cap, y = surv, fill = HookSize_cap))+
      geom_violin()+
      geom_boxplot()+
      coord_cartesian(ylim = c(.9,1.1))
  }
  if("Barb_cap" %in% covars){
    gear_barb_plot = ggplot(data = surv_dist, aes(x = Gear_cap, y = surv, fill = Barb_cap))+
      geom_violin()+
      geom_boxplot()+
      coord_cartesian(ylim = c(.9,1.1))
    
  }
  gear_plot = ggplot(data = surv_dist, aes(x = Gear_cap, y = surv))+
    geom_violin()+
    geom_boxplot()+
    coord_cartesian(ylim = c(.9,1.1))
}
...
#Issue: finding the average Run year, DOY, and survey ID cap - do these need to be integrated out as done by brmsmargins?
# marg_eff = marginaleffects(brm_coho, type = "response")
# 
# marg_b = as.tibble(marg_eff) %>%
#   dplyr::select(conf.low:Barb_cap) %>%
#   group_by(Gear_cap, Barb_cap)%>%
#   dplyr::summarize(mean_rec = mean(predicted),
#                    l95_rec = mean(predicted_lo),
#                    u95_rec = mean(predicted_hi)) %>%
#   ungroup() %>%
#   mutate(control_mean = marg_b$mean_rec[1],
#          control_low = marg_b$l95_rec[1],
#          control_high = marg_b$u95_rec[1]) %>%
#   mutate(rel_surv = mean_rec/control_mean,
#          rel_low = l95_rec/control_low,
#          rel_high = u95_rec/control_high) #%>%
#   #dplyr::select()
# 
# #Barb
# mbarb = brmsmargins(brm_coho, at = data.frame(Barb_cap = c( "Control","No","Yes")), effects = "integrateoutRE", k = 100L, seed = 425)
...
#   xlim(0.9,1.1)
# #gear
# mgear = brmsmargins(brm_coho, at = data.frame(Gear_cap = c("Control", "Bait","Jig","Lure")), effects = "fixedonly")
#   
...
covars = c("Gear_cap","Method_cap")
modname = "gearmethod"
formula = formula(crit_resp ~ Gear_cap:Method_cap)
if(!file.exists(paste0("results/coho_crithook_model_",modname,".rds"))){
...
saveRDS(brm_cohocrit, file = paste0("results/coho_crithook_model_",modname,".rds"))

}else{
  brm_cohocrit = readRDS(paste0("results/coho_crithook_model_",modname,".rds"))
}

summary(brm_cohocrit)

mod_effplots = plot(conditional_effects(brm_cohocrit), points = T, ask = F)


cov_comb = covComb(dat=cdat_treat, covars)%>%
  filter(Gear_cap != "Control")
comb_names = apply(cov_comb, 1, paste, collapse = "_")
...
meff_quant = surv_dist %>%
  group_by(name) %>%
  dplyr::summarize(med = quantile(value, .5),
...
save(mod_effplots, meff_quant,file = paste0("results/coho_crithook_model_",modname,".rds"))
```

```{r Coho-data-summaries}
cdat = dat%>%
  filter(SpeciesCode_cap=="COHO")
...
```{r BRMS-S-steelhead-treatmentonly}
cdat = dat %>%
  filter(SpeciesCode_cap=="STLHD" & Treatment_cap %in% c("Treatment"))

modname = c("treatmentfish_full")[1]
priorname = c("norm","hs", "hs_secondaryeffs")[2]
re = c("(1| survey_ID_cap)", "s(DOY_cap,run_year_cap,bs='fs')")
covars = c(treatvars[-c(1,2,8,11)], "CritHook_cap", "FL_cm_cap", "Temp_C_cap")
cov_form = c(treatvars[-c(1,2,8,11)],"CritHook_cap", "FL_cm_cap", "Temp_C_cap")
formula = bf((paste('rec ~ 1 + ', paste(re, collapse = ' + '),'+', paste(cov_form, collapse = ' + '))))
...
```{r BRMS-W-steelhead}
cdat = dat %>%
  filter(SpeciesCode_cap=="WSTLHD")



```

```{r BRMS-W-steelhead-treatmentonly}
cdat = dat %>%
...
```{r W-steelhead-BDanalysis}
cdat = dat %>%
  filter(SpeciesCode_cap=="WSTLHD" & Treatment_cap %in% c("Treatment")) %>%
  mutate(bd_ind = ifelse(Method_cap == "BD", 1,0),
         Gear_capBait = ifelse(Gear_cap == "Bait", 1,0),
         Gear_capFly = ifelse(Gear_cap == "Fly", 1,0),
         Gear_capJig = ifelse(Gear_cap == "Jig", 1,0),
         CritHook_ind = ifelse(CritHook_cap == "Critical", 1,0))

covars = c("Gear_capBait","Gear_capFly","Gear_capJig", "CritHook_ind", "HookLeftinFish_cap")
cov_form = c("Gear_capBait","Gear_capFly","Gear_capJig", "CritHook_ind", "HookLeftinFish_cap")
formula = bf((paste('bd_ind ~ 1 + ','+', paste(cov_form, collapse = ' + '))))

if(!file.exists(paste0(here::here(),"/results/","WinterSTHD_gearcapBD_model.rds"))){
  
wsthd_bd = brms::brm(formula = formula
          ,data=cdat
          ,family = bernoulli(link="logit")
          #,method = "REML"
          ,cores = 4
...
saveRDS(wsthd_bd, paste0(here::here(),"/results/","WinterSTHD_gearcapBD_model.rds"))


}else{readRDS(paste0(here::here(),"/results/","WinterSTHD_gearcapBD_model.rds"))
}
summary(wsthd_bd)

mod_effplots = plot(conditional_effects(wsthd_bd), points = T, ask = F)
```
...
```{r GAM analysis SCHK, results="hold",echo = T, message = T, warning = FALSE }
cdat<-dat%>%
...
```{r BRMS analysis SCHK, results="hold",echo = T, message = T, warning = FALSE }
# treatvars = dat %>%
#   dplyr::select(FightTime_seconds_cap:AnglerCode_cap, HookLeftinFish_cap) %>%
#   names()
# 
...
#Model run and output process
#Designate model name
modname = c("treatonly","treat_gear", "treat_gear_barb","treat_gear_barb","gear_barb", "fullCat","full")[4]
priorname = c("norm","hs", "hs_secondaryeffs")[2]
re = c("(1| survey_ID_cap)", "s(DOY_cap,run_year_cap,bs='fs')")
covars = c("Treatment_cap","Gear_cap", "Barb_cap")
cov_form = c("Treatment_cap", "Gear_cap","Barb_cap")
formula = bf((paste('rec ~ 1 + ', paste(re, collapse = ' + '),'+', paste(cov_form, collapse = ' + '))))
# formula = bf(rec ~ 1 + (1| survey_ID_cap) + s(DOY_cap,run_year_cap,bs='fs') + a + b, nl = T) +
#   lf(a ~ Gear_cap:Barb_cap, center = T) +
#   lf(b ~ 0 + Treatment_cap, cmc = F)
...
if(!file.exists(paste0(here::here(),"/results/", "schk_", modname, "_", priorname, "_modeloutputs", ".rda"))){
...
if("Gear_cap" %in% covars){
  if("HookSize_cap" %in% covars){
    gear_hooksize_plot = ggplot(data = surv_dist, aes(x = Gear_cap, y = value, fill = HookSize_cap))+
      geom_violin()+
      geom_boxplot()
  }
  if("Barb_cap" %in% covars){
    gear_barb_plot = ggplot(data = surv_dist, aes(x = Gear_cap, y = value, fill = Barb_cap))+
      geom_violin()+
      geom_boxplot()
...
save(file = paste0(here::here(),"/results/","schk_",modname,"_",priorname,"_outputs",".rda"),
     list = intersect(ls(),c("meff_dist",
                      "surv_quant",
                      "mod_effplots",
                      "treat_plot",
