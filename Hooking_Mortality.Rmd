...
packages_list<-c("plyr","tidyverse","dplyr","lubridate","tidyr","scales","RODBC","MuMIn","RColorBrewer","reshape2","ggplot2","gplots","mgcv","sjPlot","sjmisc","gridExtra","modelr","kableExtra","dataRetrieval","MASS","brms","rstan","sf","riverdist","maptools","rgdal","raster","rgeos","devtools","webshot","phantom","marginaleffects", "brmsmargins", "here", "purrr") 
...
modname = c("treatonly","treat_gear", "treat_gear_barb", "full")[4]
priorname = c("norm","hs", "hs_secondaryeffs")[2]
re = c("(1| survey_ID_cap)", "s(DOY_cap,run_year_cap,bs='fs')")
covars = c("Treatment_cap", "Gear_cap", "Barb_cap", "Temp_C_cap", names(cdat)[c(11:17,19:20)])
cov_form = c("Treatment_cap", "Gear_cap", "Barb_cap", "Temp_C_cap", names(cdat)[c(11:17,19:20)])
...
if(!file.exists(paste0(here(),"/results/","coho_", modname,"_",priorname,".rds"))){
...
brm_coho = brms::brm(formula = formula
          ...
          #,thin = 5
          ,warmup = 1000
          ,prior = prior(horseshoe(df = 1, par_ratio = 0.2), class = "b")
    )
 #save results
saveRDS(brm_coho, file = paste0(here(),"/results/","coho_", modname,"_",priorname,".rds"))
}else{
  brm_coho<-readRDS(paste0(here(),"/results/","coho_", modname,"_",priorname,".rds"))
}

#data summaries, predictions, tables, figures
summary(brm_coho)

if(!file.exists(paste0(here(),"/results/", "coho_", modname, "_", priorname, "_modeloutputs", ".rda"))){
mod_effplots = plot(conditional_effects(brm_coho), points = T, ask = F)

#Marginal effects using brmsmargins
#generate df of effects

cov_comb = covComb(cdat, covars[-5])
...
save(file = paste0(here(),"/results/","coho_",modname,"_",priorname,"_outputs",".rda"),
  ...
  load(paste0(here(),"/results/","coho_",modname,"_",priorname,"_outputs",".rda"))
}
...
#formula = 
modname = c("treatonly", "gear_barb", "full")[1]
priorname = c("hs", "hs_secondaryeffs")[1]
re = c("(1| survey_ID_cap)", "s(DOY_cap,run_year_cap,bs='fs')")
covars = c("Treatment_cap", "Gear_cap")
formula = as.formula(paste('rec ~ 1 + ', paste(re, collapse = ' + '),'+', paste(covars, collapse = ' + ')))

#Check if this model permutation has been run, and if so load the output
if(!file.exists((paste0("results/", modname,"_",priorname,".rds")))){

brm_schk = brms::brm(formula
          ,family = bernoulli(link="logit")
          ,data=cdat
          #,method = "REML"
          ,cores = 4
          ...
          #,thin = 5
          ,warmup = 1000
          ,prior = set_prior(horseshoe(df = 1, par_ratio = 0.5), class="b") #horseshoe or similar prior is required, fixed effects do not converge with diffuse priors (eg normal, uniform) 
    )
#save results
saveRDS(brm_schk, file = here(paste0("results/","schk_", modname,"_",priorname,".rds")))
}else{
  brm_schk<-readRDS(here(paste0("results/","schk_", modname,"_",priorname,".rds")))
}

#data summaries, predictions, tables, figures
summary(brm_schk)

if(!file.exists(here(paste0("results/",modname,"_",priorname,"_modeloutputs",".rda")))){
...
save(file = here(paste0("results/","schk_",modname,"_",priorname,"_outputs",".rda")),
     list = intersect(ls(),c("meff_dist",
                      "meff_quant",
                      "mod_effplots",
                      "treat_plot",
  ...
  load(here(paste0("results/","schk_",modname,"_",priorname,"_outputs",".rda")))
}
