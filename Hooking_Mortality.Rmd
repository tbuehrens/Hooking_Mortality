...
packages_list<-c("tidyverse", "plyr","dplyr", "lubridate","tidyr","scales","RODBC","MuMIn","RColorBrewer","reshape2","ggplot2","gplots","mgcv","sjPlot","sjmisc","gridExtra","modelr","kableExtra","dataRetrieval","MASS","brms","rstan","sf","riverdist","maptools","rgdal","raster","rgeos","devtools") 
...
cdat<-dat%>%
  ...
  mutate(Gear_cap = ifelse(is.na(Gear_cap), "Control", Gear_cap)) %>%
  mutate(run_year_cap = as.factor(run_year_cap),
         Treatment_cap = as.factor(Treatment_cap),
         Gear_cap = factor(Gear_cap, levels = c("Con", "B", "BL", "J","F","L"))) %>%
  filter(Gear_cap != "BL")
...
GL<-GaussLobatto(20)
cdat2<-GAMSurvdataset(GL,data.frame(cdat),which(colnames(cdat)=="time"),which(colnames(cdat)=="rec"))

fGAM<-gam(gam.ev~
            s(stop,bs="cr")+
            as.factor(Treatment_cap)+
            offset(log(gam.dur)),
          data=cdat2,family="poisson",scale=1,method="REML")

summary(fGAM)
par(mfrow=c(2,1))
plot(fGAM,main="Gauss Lobatto (20)",ylab="log-basehaz")
plot(fGAM,main="Gauss Lobatto (5)",ylab="basehaz",trans=exp)
#next: https://statmd.wordpress.com/2015/05/03/survival-analysis-with-generalized-additive-models-part-iv-the-survival-function/


#Poisson gam for recovery time

pois.gam.crec = gam(time ~ 
          s(run_year_cap,bs="re") 
          + s(DOY_cap,by=run_year_cap,k=20,m=1)
          + s(survey_ID_cap,bs="re",m=1)
          + Treatment_cap
          ,family = 'poisson'
          ,data=filter(cdat, rec == 1)
    )
summary(pois.gam.crec)
